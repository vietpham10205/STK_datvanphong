...
public class BookingDAO {
    ...
    String sql = "SELECT * FROM BOOKINGS WHERE ROOM_ID = ? ORDER BY BOOKING_ID DESC FETCH FIRST 1 ROWS ONLY";
    try (Connection conn = OracleConnection.getConnection();
         PreparedStatement pstmt = conn.prepareStatement(sql)) {
        pstmt.setInt(1, roomId);
        ResultSet rs = pstmt.executeQuery();
    ...
    String sql = "SELECT * FROM BOOKINGS WHERE ROOM_ID = ? AND (STATUS = 'Chờ xác nhận' OR (STATUS = 'Đã xác nhận' AND CHECK_OUT >= SYSDATE)) ORDER BY STATUS DESC, BOOKING_ID DESC FETCH FIRST 1 ROWS ONLY";
    try (Connection conn = OracleConnection.getConnection();
         PreparedStatement pstmt = conn.prepareStatement(sql)) {
        pstmt.setInt(1, roomId);
        ResultSet rs = pstmt.executeQuery();
